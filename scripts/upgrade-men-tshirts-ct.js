...
async function run() {
  const ct = await getCT('men_tshirts')
  const s = ct.schema

  // Add parity fields similar to men_shirts
  ...
  await axios.put(`${BASE}/content_types/men_tshirts`, { content_type: ct }, { headers })
  console.log('✅ Upgraded men_tshirts content type with parity fields')
}

run().catch((e) => {
  console.error('❌ Failed to upgrade men_tshirts:', e.response?.data || e.message)
  process.exit(1)
})
