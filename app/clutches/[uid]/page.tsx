...
export default async function Page({ params }: { params: { uid: string } }) {
  const entry: any = await fetchEntryByUid<any>('clutches', params.uid)
    ...
    <main className="min-h-screen bg-white">
      <div className="container mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div>
          ...
          <AddToCartButton item={{ uid: entry.uid, title: entry.title, price: entry.price, imageUrl: img, contentTypeUid: 'clutches' }} className="mb-6 px-5 py-3 bg-luxury-gold text-luxury-black font-semibold rounded" />
        </div>
      </div>
    </main>
  )
