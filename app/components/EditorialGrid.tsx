...
export type EditorialCardData = { headline: string; sub: string; imageUrl?: string; dark?: boolean; ctaUrl?: string; ctaLabel?: string }

const EditorialCard = ({ headline, sub, imageUrl, dark, ctaUrl, ctaLabel, index }: EditorialCardData & { index: number }) => {
  const label = ctaLabel || 'Discover'
  // Prefer CMS url if provided; otherwise map by index:
  // 0 -> Men Jackets, 1 -> Men Shirts
  const derivedUrl = ctaUrl || (index === 0 ? '/men-jackets' : index === 1 ? '/men-shirts' : '/')
  return (
    <motion.div
    ...
    >
      ...
      <div className="absolute inset-0 p-8 flex flex-col justify-end">
        ...
        <a href={derivedUrl} className="inline-block">
          <motion.button whileHover={{ x: 2 }} className={`self-start px-5 py-2 rounded-md font-semibold tracking-wide ${dark ? 'bg-white text-gray-900' : 'bg-gray-900 text-white'} hover:opacity-90`}>{label}</motion.button>
        </a>
      </div>
    ...
    <section className="py-20 bg-white">
      <div className="container mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {(cards && cards.length > 0
          ...
          <EditorialCard key={c.headline} index={idx} {...c} />
        ))}
      </div>
    </section>
  )
