...
export default async function ProductDetailByUid({ params, contentTypeUid }:{ params: { uid: string }, contentTypeUid: string }) {
  const entry: any = await fetchEntryByUid<any>(contentTypeUid, params.uid)
    ...
    <main className="min-h-screen bg-white">
      <div className="container mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div>
          ...
          <AddToCartButton
            item={{ uid: entry.uid, title: entry.title, price: entry.sale_price ?? entry.price, imageUrl: img, contentTypeUid }}
